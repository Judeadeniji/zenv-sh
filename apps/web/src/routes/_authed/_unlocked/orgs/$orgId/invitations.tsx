    ...
    <div>
      ...
      <DataTable
        ...
        emptyAction={
          <InviteMemberDialog
            orgId={orgId}
            trigger={<Button type="button" size="sm"><UserPlus /> Send an invitation</Button>}
          />
        }
      />
...
function InvitationDetailsSheet({ orgId, invitation, onClose }: InvitationDetailsSheetProps) {
  ...
  return (
    <Sheet open={!!invitation} onOpenChange={(open) => { if (!open) onClose() }}>
      <SheetContent>
        <SheetHeader>
...
function PageHeader() {
  return (
    <div>
      ...
      <p className="mt-1 text-sm text-muted-foreground">Manage pending requests for people to join your organization.</p>
    </div>
  )
}