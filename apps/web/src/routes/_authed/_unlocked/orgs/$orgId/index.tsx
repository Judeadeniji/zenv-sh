...
function OrgDashboard() {
	...
	const { data: projectsData, isLoading: projectsLoading } = useQuery(projectsQueryOptions(orgId))
	const { data: membersData, isLoading: membersLoading } = useQuery(orgMembersQueryOptions(orgId))

	const orgName = (org as { name?: string })?.name ?? "Organization"
	const projects: { id: string; name: string }[] =
		(projectsData as { projects?: { id: string; name: string }[] })?.projects ?? []
	const members: { id: string; email?: string; name?: string; role?: string; created_at?: string }[] =
		(membersData as { members?: { id: string; email?: string; name?: string; role?: string; created_at?: string }[] })?.members ?? []
	...
	return (
		<div>
			...
			<div className="mb-6">
				...
				<p className="mt-0.5 text-xs text-muted-foreground">Organization overview</p>
			</div>
						...
						<ActionCard className="cursor-pointer transition-all hover:scale-[1.02] shadow-none border">
							<div className="flex items-center gap-3">
								...
								<div className="min-w-0">
									...
									<p className="text-[11px] text-muted-foreground">Add a secret vault</p>
								</div>
							</div>
						</ActionCard>
					}
						...
						<ActionCard className="cursor-pointer transition-all hover:scale-[1.02] shadow-none border">
							<div className="flex items-center gap-3">
								...
								<div className="min-w-0">
									...
									<p className="text-[11px] text-muted-foreground">Grow your team</p>
								</div>
							</div>
						</ActionCard>
					}
					...
					<ActionCard className="h-full cursor-pointer transition-all hover:scale-[1.02] shadow-none border">
						<div className="flex items-center gap-3">
							...
							<div className="min-w-0">
								...
								<p className="text-[11px] text-muted-foreground">Manage your org</p>
							</div>
						</div>
					</ActionCard>
				</Link>
					...
					{members.slice(0, 5).map((m) => (
						<div key={m.id} className="flex items-center gap-3">
							...
							<div className="min-w-0 flex-1">
								<p className="truncate text-sm font-medium">{m.name || m.email || "Unnamed"}</p>
							</div>
							<Badge variant={m.role === "owner" ? "primary" : m.role === "admin" ? "primary" : "neutral"} className="text-[10px]">
								{m.role}
							</Badge>
						</div>
					))}
...
function ProjectRow({ orgId, project }: { orgId: string; project: { id: string; name: string } }) {
	...
	const secretCount = ((secretsData as { secrets?: unknown[] })?.secrets ?? []).length
	const tokenCount = ((tokensData as { tokens?: unknown[] })?.tokens ?? []).length
		...
		>
			...
			<div className="flex items-center gap-3 text-xs text-muted-foreground">
				<span className="flex items-center gap-1">
					<KeyRound className="size-3" /> {secretCount}
				</span>
				<span className="flex items-center gap-1">
					<FileKey className="size-3" /> {tokenCount}
				</span>
			</div>
	...
	return (
		<div className="mb-6 flex flex-col items-center justify-center rounded-lg border border-dashed border-border py-12 text-center">
			...
			<p className="mt-1 max-w-xs text-xs text-muted-foreground">
				Projects hold your encrypted secrets, organized by environment. Create one to get started.
			</p>
			<div className="mt-4">
				<CreateProjectDialog
					orgId={orgId}
					trigger={<Button type="button" size="sm"><Plus /> Create your first project</Button>}
				/>
			</div>
		</div>
	)
