				...
				<div className="flex items-end justify-between">
					<div>
						...
						<p className="mt-1 text-xs text-muted-foreground">
							<span className="tabular-nums">{projects.length}</span> project
							{projects.length !== 1 ? "s" : ""}
							{" · "}
							<span className="tabular-nums">{members.length}</span> member
							{members.length !== 1 ? "s" : ""}
						</p>
					</div>
					...
					<div className="flex items-center gap-1.5">
						<InviteMemberDialog
							orgId={orgId}
							trigger={
								<Button size="sm" className="h-8 gap-2 border-border text-xs font-normal">
						...
						{members.map((m, i) => (
							...
							>
								...
								<div className="min-w-0 flex-1">
									<p className="truncate text-sm leading-none">{m.user.name}</p>
...
function RoleBadge({ role }: { role?: string }) {
	...
	return <span className="px-2 py-0.5 text-[10px] text-muted-foreground">member</span>
}
