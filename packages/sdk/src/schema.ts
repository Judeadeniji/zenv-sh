...
function getObjectShape(schema: Record<string, unknown>): Record<string, unknown> | null {
	...
	if ("entries" in schema && typeof schema.entries === "object" && schema.entries) {
...
export function pickSchema(
	...
	if ("pick" in schema && typeof schema.pick === "function") {
		return schema.pick(Object.fromEntries(keys.map((k) => [k, true])))
	}

	// Valibot/ArkType or plain objects: manually pick from shape
	const shape = getObjectShape(schema)
	if (shape) {
		return Object.fromEntries(keys.filter((k) => k in shape).map((k) => [k, shape[k]]))
	}

	return Object.fromEntries(keys.filter((k) => k in schema).map((k) => [k, schema[k]]))
}
...
export type InferSchema<T> =
	T extends StandardSchemaV1<infer O>
		? O
