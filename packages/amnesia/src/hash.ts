...
import { AUTH_KEY_HASH_LENGTH, AUTH_KEY_SALT_SIZE, PASSPHRASE_PARAMS } from "./constants.ts"
 ...
 */
export async function hashName(name: string, hmacKey: Uint8Array): Promise<Uint8Array> {
...
export async function hashAuthKey(authKey: Uint8Array): Promise<Uint8Array> {
	const fullHash = new Uint8Array(await crypto.subtle.digest("SHA-256", toBuffer(authKey)))
